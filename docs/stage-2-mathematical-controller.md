# Stage 2: Mathematical Controller
...
- `2`: Mathematical Controller control.

The game still uses the same V-HAL in both modes. The controller never teleports the
paddle; it only sends target millimeter coordinates to the V-HAL.

## Algorithm

Each frame, the Mathematical Controller reads:
...
upward, the controller first reflects the trajectory off the top boundary and then
...
If the ball is traveling relatively perpendicular to the paddle, the controller checks
...
the controller intentionally targets the paddle slightly off-center. This creates a
...
correct target chosen by the Mathematical Controller.

## Data Harvesting

While the game is in Mathematical Controller mode and the ball is actively playing, a
...
`target_paddle_mm` represents the controller's desired paddle-center target. For
...
- Brick collisions can change the ball vector after the controller has predicted a
  path. The prediction is recalculated every frame, so the target corrects
  immediately after the collision.
- The controller is deterministic geometry, not perception. It has direct access to
